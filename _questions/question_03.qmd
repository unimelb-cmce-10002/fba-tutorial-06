...
- (Optional stretch) Understand the “excess win rate” idea (actual – fair-chance expected) and why it’s sensible but a step up.

✅   **Core Concepts to Highlight**

- Data model & keys: bids (events) join to auctions → items → seller_id; orders hold winner_id.
...
“Say in one sentence how a database differs from CSV/JSON for our purposes.”

“Why must win rate use distinct auctions?” (Avoids ‘more bids = higher win rate’ confusion.)

“Give one benign and one review reason for strong bidder–seller ties.”

“On the scatter (win_rate vs seller_share), point to the review zone and justify a simple rule.”
...
Useful for shill review: repeated bidder↔seller ties across many auctions can suggest coordination or price support for that seller.
...
It normalises for activity: a single number (0–1) that's comparable across bidders, regardless of total bids.
...
Use a relative cutoff, not an absolute one: e.g., above 2× the median seller_share (as in the scaffold).


Could add: 

- Add a minimum activity guard (e.g., ≥ 8–10 auctions) to avoid small-sample noise.
...
High denominator: very active bidders can still devote hundreds of bids to one seller while the percentage stays ~20–30%.
...
High concentration + Low win rate: strongest review zone (focused on one seller but rarely wins → price-pushing pattern).
...
For each auction with  k bidders, a neutral, fair-chance win rate is  1/k. For a bidder who joins many auctions, take the average of 1/k over their auctions → that's their expected win rate.

Then compute excess win rate = actual win rate − expected win rate. Look for situations where win_rate a lot less (how to measure / relative to what? perhaps a metric of standard deviation of win rate, or position in a distribution) than the expected win rate.

Why it's sensible:

- Adjusts for crowding (harder to win big auctions).
- Lets “0.25” mean different things when you usually face 3 rivals vs 20.
...
- Flag if excess win ≤ −0.15 (under-winning vs fair chance) and seller_share ≥ 0.60, with ≥ 10 auctions.

Manager line: “These bidders focus on one seller yet win less than a fair-chance baseline—good review candidates (not proof). We'd check bid timelines and device/IP overlap next.”
