...
Why it's used: handles large files smoothly because you can read it line by line without loading everything.
...
- Use simple telemetry signals (shared IPs, auto-bid share) to create a review list of auctions, clearly distinguishing suggestive patterns from evidence.

✅   **Core Concepts to Highlight**

- Streaming JSON vs JSON: one record per line; load with `stream_in()`; then `unnest_wider()` to flatten nested objects.

- Nesting → columns: bidder.* → bidder_id, bidder_rating, bidder_country; device.* → device_os, device_browser.

- Granularity matters: summaries at the auction level vs auction–IP vs auction–bidder will change row counts and joins.
...
- Flags ≠ verdicts: manager-ready language should emphasise “review”/“follow-up”, not accusations.

💬 **Suggested In-Class Prompts** (if needed)

“In one or two sentences, describe what is at the top level of the JSON vs. what's nested.”

“Give one benign and one suspicious reason for multiple bidders sharing an IP in the same auction.”

“When might a high share of auto-bids be normal? When might it warrant a review?”

📌 **Common Misunderstandings**

Using `read_json()` on streaming JSON → fails or loads only first line. Use `stream_in()` and then unnest_wider().

Not flattening nested fields → students can't reference bidder_id/device_os directly.

Granularity mistakes in joins → duplicate rows when joining per-IP tables to per-auction tables. Remedy: summarise to the intended level first.

Assuming “same IP = same person.” Emphasise shared networks and VPNs; treat as a weak, noisy signal.
